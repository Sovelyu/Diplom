...
if __name__ == '__main__':
    ...
    u,v = computeHS(args.img1, args.img2, alpha = 15, delta = 1e-4)

    path = os.path.join(os.path.dirname(__file__), 'test images')
    beforeImg = cv2.imread(os.path.join(path, args.img1), cv2.IMREAD_GRAYSCALE)
    afterImg = cv2.imread(os.path.join(path, args.img2), cv2.IMREAD_GRAYSCALE)
    beforeImg = cv2.GaussianBlur(beforeImg, (5, 5), 0)
    afterImg = cv2.GaussianBlur(afterImg, (5, 5), 0)
